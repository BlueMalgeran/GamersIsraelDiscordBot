...
client.on("guildCreate", guild => {
    const logsServerJoin = client.channels.get('428728613970378763');
...
client.on("guildDelete", guild => {
    const logsServerLeave = client.channels.get('428728613970378763');
...
client.on("message", async message => {
    ...
    const logsCommands = client.channels.get('428728613970378763');
    ...
    switch (args[0]) {
      ...
      // Let adminRoleMute = guild.roles.find("name", "TOA");
        ...
        console.log(`${message.author.tag} used the ${settings.botPREFIX}unmute command!`);
            logsCommands.send(`${message.author.tag} used the ${settings.botPREFIX}unmmute command!`);
...
message.channel.send({embed: {
    ...
    footer: {
        ...
        case "warn":
            console.log(`${message.author.tag} used the ${settings.botPREFIX}warn command!`);
            logsCommands.send(`${message.author.tag} used the ${settings.botPREFIX}warn command!`);
...
**${settings.botPREFIX}say** - Give the bot something to say!`
    ...
    break;
        ...
        default:
            message.channel.send(`Invalid command. Prefix \`${settings.botPREFIX}\``);
    }
});
